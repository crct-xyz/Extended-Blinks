...
import { useCluster } from 'providers/cluster/cluster-data-access'
import { ExplorerLink } from 'providers/cluster/cluster-ui'
import {
    useGetBalance,
    useGetSignatures,
    useGetTokenAccounts,
    ...
    if (query.isError || !query.data) {
        return (
            <div className="alert alert-warning text-warning-content/80 flex justify-center rounded-none">
                <span>
                    You are connected to <strong>{cluster.name}</strong> but
    ...
    return (
        <AppModal
            ...
            submit={() =>
                mutation
                    //@ts-ignore
                    .mutateAsync(Number.parseFloat(amount))
                    .then(() => hide())
            }
        >
            <input
